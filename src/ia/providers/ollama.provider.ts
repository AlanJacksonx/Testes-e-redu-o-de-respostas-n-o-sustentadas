...
import { GerarRespostaInput, GerarRespostaOutput, GerarStreamInput, ModeloProvider } from './modelo.provider';

interface OllamaChatResponse { model: string; message?: { role: string; content: string; }; done: boolean; prompt_eval_count?: number; eval_count?: number; }
interface OllamaStreamChunk { message?: { content?: string; }; done: boolean; error?: string; }

function parseOllamaLine(line: string): OllamaStreamChunk | undefined {
  const normalized = line.trim();
  if (!normalized) return undefined;
  const parsed: unknown = JSON.parse(normalized);
  if (typeof parsed !== 'object' || parsed === null) throw new BadGatewayException('Fragmento invalido recebido do modelo');
...
export class OllamaProvider implements ModeloProvider {
  ...
  async gerar(input: GerarRespostaInput): Promise<GerarRespostaOutput> {
    ...
    try {
      const response = await this.http.axiosRef.post<OllamaChatResponse>(
        `${baseUrl}/api/chat`, { model, messages: [{ role: 'user', content: input.mensagem }], stream: false }, { timeout }
      );
      const content = response.data.message?.content?.trim();
      if (!content) throw new BadGatewayException('Resposta invalida do modelo');
      return { resposta: content, modelo: response.data.model, tokensEntrada: response.data.prompt_eval_count, tokensSaida: response.data.eval_count };
    } catch (error: unknown) {
      if (error instanceof BadGatewayException) throw error;
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') throw new GatewayTimeoutException('Tempo limite excedido');
        if (error.code === 'ECONNREFUSED') throw new ServiceUnavailableException('Servidor indisponivel');
      }
  ...
  async *gerarStream(input: GerarStreamInput): AsyncIterable<string> {
    ...
    const response = await this.http.axiosRef.post<NodeJS.ReadableStream>(
      `${baseUrl}/api/chat`,
      { model, messages: [{ role: 'user', content: input.mensagem }], stream: true },
      { responseType: 'stream', signal: input.signal, timeout }
    );
