...
async function main(): Promise<void> {
  ...
  try {
    ...
    await writeFile('resultado-avaliacao.json', JSON.stringify(relatorio, null, 2));
    console.table(relatorio.resultados);
    console.log({ total: relatorio.total, acuracia: relatorio.acuracia, conformidade: relatorio.conformidadeFormato });
  } finally {
    await app.close();
  }
}
